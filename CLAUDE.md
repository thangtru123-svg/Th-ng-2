...
- **Xưng hô**: Gọi người dùng là **"đại nhân"**, tự xưng là **"con"**, mở đầu câu bằng **"Bẩm đại nhân"**
...
| Mục | Giá trị |
|-----|---------|
| Website (VERCEL — DÙNG CÁI NÀY) | https://th-ng-2.vercel.app |
| Website cũ (Netlify — hết credit) | https://xbg-dashboard.netlify.app |
...
4. Commit + Push qua GitHub Desktop → **Vercel tự động deploy** (không cần làm gì thêm)

---

## Quy tắc chụp ảnh gửi Telegram

### ⚠️ QUAN TRỌNG — Tránh nhầm table dropdown search

Tất cả dashboard có `addBCSearch` đều có **2 tables** trong cùng 1 element: `table[0]` = dropdown search (hidden, `width:100%`, `scrollWidth=0`), `table[1]` = data table (`width:Xpx`, visible).

**LUÔN dùng pattern này thay vì `querySelector('table')`:**
- Lấy data table: `el.querySelector('.table-wrap>table')` hoặc `[...el.querySelectorAll('table')].find(t=>t.offsetWidth>0)`
- Lấy tbody để trim rows: `dataTable.querySelector('tbody')` (KHÔNG phải `el.querySelector('tbody')`)
- Đã fix trong: `_captureEl` (`.table-wrap>table` ưu tiên), `sendAgingToTelegram` trim BC, `sendAllToTelegram` trim BC
...
| Lỗi | Xử lý |
|-----|-------|
| GitHub Pages CDN bị stuck (deployment_queued mãi) | Đã chuyển sang Vercel — KHÔNG cần sửa GitHub Pages nữa |
...
| `_captureEl` chụp khoảng trắng / scrollWidth=0 | `querySelector('table')` lấy nhầm dropdown search table (width:100%, hidden) thay vì data table — phải dùng `.table-wrap>table` ưu tiên |
| Trim BC rows nhầm tbody của dropdown | `querySelector('#id tbody')` lấy tbody đầu tiên = tbody dropdown — phải dùng `querySelector('#id .table-wrap>table tbody')` hoặc `[...querySelectorAll('#id table')].find(t=>t.offsetWidth>0).querySelector('tbody')` |
